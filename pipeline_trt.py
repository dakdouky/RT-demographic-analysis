...
model_path = "/home/toolkit/users/dakdouky_ws/RT-demographic-analysis/demographic_analysis/best_model.pth"
classifier = MultiTaskFaceClassifier(model_path)

# Convert the classifier model to TorchScript
classifier.model.eval()
example_input = torch.randn(1, 3, 224, 224).to(classifier.device)  # Example input tensor
traced_model = torch.jit.trace(classifier.model, example_input)

# Optimize the model using Torch-TensorRT
trt_model = torch_tensorrt.compile(
    traced_model,
    ir='torchscript',
    inputs=[torch_tensorrt.Input((1, 3, 224, 224))],
    enabled_precisions={torch.half}  # Use torch.half for FP16
)
...
try:
    ...
    for i in range(num_faces):
        ...
        face_tensor = torch.from_numpy(face_resized).permute(2, 0, 1).unsqueeze(0).float().to(classifier.device)

        # Measure Age & Gender Classification Inference Time & GPU Usage
        torch.cuda.synchronize()
        start_time = time.time()
        with torch.no_grad():
            logits = trt_model(face_tensor)
            print(logits)
            age_label, gender_label, ethnicity_label = classifier.postprocess_trt(logits)
        torch.cuda.synchronize()
        end_time = time.time()
